...
*/
func RecordReduceWithKey(records interface{}, outputRecord interface{}, indexing ...string) error {
	if reflect.TypeOf(outputRecord).Kind() != reflect.Ptr || reflect.Indirect(reflect.ValueOf(outputRecord)).Kind() != reflect.Map {
		return errors.New("outputRecord must be a pointer to a map")
	}

	if reflect.Indirect(reflect.ValueOf(outputRecord)).IsNil() {
		return errors.New("outputRecord cannot be nil pointer")
	}
	output := reflect.Indirect(reflect.ValueOf(outputRecord))

	recordsType := reflect.TypeOf(records)

	if recordsType.Kind() != reflect.Slice {
		return errors.New("records is not a slice, got: " + recordsType.Kind().String())
	}
	recordsVal := reflect.ValueOf(records)

	for i := 0; i < recordsVal.Len(); i++ {
		record := recordsVal.Index(i)
		currentLevel := output
		...
		for i, prop := range indexing {
			...
			tPropValue = record.FieldByName(prop)
			if !tPropValue.IsValid() {
				return errors.New("invalid prop name of " + prop)
			}
			tPropKind := record.FieldByName(prop).Kind()
			switch tPropKind {
			...
			if len(indexing) == i+1 {
				...
				if currentLevel.Type().Elem().Kind() == reflect.Slice {
					if !found {
						...
						if record.Type() != nextSliceType.Elem() {
							return errors.New("cannot assign slice's value of datatype " + nextSliceType.Elem().String() + " for record of type " + record.Type().String())
						}
						currentLevel.SetMapIndex(tPropValue, reflect.Append(reflect.MakeSlice(nextSliceType, 0, 0), record))
					} else {
						currentLevel.SetMapIndex(tPropValue, reflect.Append(currentLevel.MapIndex(tPropValue), record))
					}
				} else {
					if record.Type() != currentLevel.Type().Elem() {
						return errors.New("cannot assign map's value of datatype " + currentLevel.Type().Elem().String() + " for record of type " + record.Type().String())
					}
					currentLevel.SetMapIndex(tPropValue, record)
				}
			}
